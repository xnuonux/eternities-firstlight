...
import argparse, hashlib, json, threading, time, platform
...
def main():
    ...
    result = {"html_sha256": expected, "origin": f"http://127.0.0.1:{server.server_port}/FIRSTLIGHT_VALLEY.html", "quality": args.quality, "renderer_requested": args.renderer, "frames_requested": args.frames, "viewport": {"width":1920,"height":1080}, "platform":platform.platform(), "method":"Normal RAF intervals, isolated headless hardware Chrome; excludes setup warmup. Not GPU render time or human qualification."}
    try:
        with sync_playwright() as pw:
            ...
            page = browser.new_page(viewport={"width": 1920, "height": 1080}); page.add_init_script("window.__ETERNITIES_TEST_MODE=true;"); response = page.goto(result["origin"], wait_until="load"); result["response_sha256"] = hashlib.sha256(response.body()).hexdigest()
            ...
            result["scenes"] = []
            for scene in scenes:
                page.evaluate(scene["expression"]); page.wait_for_timeout(1500); intervals = page.evaluate("""(n)=>new Promise(resolve=>{let a=[],last;function f(t){if(last!==undefined)a.push(t-last);last=t;if(a.length<n)requestAnimationFrame(f);else resolve(a)}requestAnimationFrame(f)})""", args.frames)
                intervals.sort(); pct=lambda p: intervals[min(len(intervals)-1, int(len(intervals)*p))]
                result["scenes"].append({"name":scene["name"],"observed_scene":page.evaluate("Realm.diagnostics.scene"),"samples":len(intervals),"diagnostics":page.evaluate("Realm.diagnostics"),"frame_time_ms":{"p50":pct(.5),"p95":pct(.95),"p99":pct(.99),"max":max(intervals)},"stutter_over_33ms":sum(x>33.333 for x in intervals),"stutter_over_50ms":sum(x>50 for x in intervals)})
