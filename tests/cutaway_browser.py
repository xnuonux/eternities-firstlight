...
import json,hashlib
R=Path(__file__).resolve().parents[1];O=R/'evidence10';rep={'method':__doc__,'engine_sha256':hashlib.sha256((R/'src/engine.js').read_bytes()).hexdigest(),'checks':[]}
with sync_playwright() as pw:
 ...
 p.set_content('<canvas id="c"></canvas><script>'+(R/'src/engine.js').read_text()+'</script>')
