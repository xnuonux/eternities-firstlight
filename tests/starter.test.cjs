...
test('new quest state is explicit, versioned and round trips',()=>{
 const s=A.fresh();assert.equal(A.VERSION,9);assert.deepEqual(s.starter,{version:1,accepted:false,bundles:[],reward:null});assert.deepEqual(A.validate(s),s);
});
test('version-five adventure migrates without altering existing campaign or creative data',()=>{
 ...
 const before=JSON.stringify(raw),out=C.validate(raw);assert.equal(out.adventure.version,9);assert.deepEqual(out.adventure.starter,A.fresh().starter);
...
test('all banked XP boundaries retain XP and the existing five-level curve',()=>{
 for(const xp of [0,29,30,79,80,149,150,259,260,9999]){const a=A.fresh();a.version=5;delete a.starter;a.xp=xp;const out=A.validate(a);assert.equal(out.xp,xp);assert.equal(out.version,9);assert.equal(A.level(out),1+[30,80,150,260].filter(n=>xp>=n).length);}
});
