...
function run(){
  ...
  if(!process.argv.includes('--sources-ready')){
    ...
    cp.execFileSync(process.execPath,['tests/starter_veteran.cjs'],{cwd:repo,stdio:'ignore'});
    cp.execFileSync(process.execPath,['tests/pursuit_journey.cjs','--veteran'],{cwd:repo,stdio:'ignore'});
  }
  ...
  let current=loaded.state,serial=0,actions=[],gameCommands=[];
  ...
  const game=(type,p,id='game')=>{const sim=new C.Simulation(current),r=sim.adventureCommand('characters-'+id+'-'+(++serial),type,p);assert.equal(r.ok,true,r.error);gameCommands.push({type,ok:true});current=sim.snapshot();save();return r;};
  const refused=(type,p,id)=>{const before=JSON.stringify(current),sim=new C.Simulation(current),r=sim.adventureCommand('characters-'+id+'-'+(++serial),type,p);assert.equal(r.ok,false);gameCommands.push({type,ok:false});assert.equal(JSON.stringify(sim.snapshot()),before);return r;};
  const core=(type,p,id='core')=>{const sim=new C.Simulation(current),r=sim.act('characters-'+id+'-'+(++serial),type,p);assert.equal(r.ok,true,r.error);gameCommands.push({type,ok:true});current=sim.snapshot();save();return r;};
  ...
  const clean=current;assert.equal(clean.adventure.pursuit.claimed,0);assert.equal(clean.adventure.pursuit.active,null);assert.equal(clean.adventure.xp,0);assert.deepEqual(clean.adventure.owned,[]);
  ...
  const bowBeforeSwitch=current;const originalChoices=bow.adventure.beacon.soul;const originalXP=bow.adventure.xp;
  ...
  game('pursuit-claim',{run:bladeRun},'blade-claim');const bladePaid=current.adventure.ore;assert.equal(bladePaid-blade.adventure.ore,3);refused('pursuit-claim',{run:bladeRun},'blade-duplicate');assert.deepEqual(current.score,blade.score);assert.deepEqual(current.retreat,blade.retreat);
  ...
  assert.deepEqual(current.score,bowBeforeSwitch.score);assert.deepEqual(current.retreat,bowBeforeSwitch.retreat);assert.deepEqual(current.visitor,bowBeforeSwitch.visitor);assert.equal(current.adventure.xp,originalXP);assert.deepEqual(current.adventure.beacon.soul,originalChoices);assert.equal(current.adventure.pursuit.active.id,bowRun);game('pursuit-claim',{run:bowRun},'bow-claim');const bowPaid=current.adventure.ore;assert.equal(bowPaid-bow.adventure.ore,3);assert.equal(current.adventure.pursuit.active,null);
  ...
  // The returning strongest campaign character keeps its identity, sockets,
  // finite fittings, XP, companion and explicit story choices through import.
  const veteran=JSON.parse(fs.readFileSync(path.join(repo,'evidence10/pursuit/veteran/07_PRACTICE_PERSISTED.json')));
  command('delete',{id:'character-2',confirmName:'River'});command('import',{world:veteran});const veteranId=store.active;
  assert.equal(veteranId,'character-5');assert.deepEqual(current,veteran);assert.equal(current.adventure.equipment.weapon,'dawn_edge');
  command('switch',{id:bowId});command('switch',{id:veteranId});assert.deepEqual(current.adventure,veteran.adventure);assert.deepEqual(current.retreat,veteran.retreat);assert.deepEqual(current.score,veteran.score);snap('04_VETERAN_RESTORED');
  command('switch',{id:bowId});
  ...
  const report={status:'passed',variant:'imported-command-earned-independent-roster',slots:store.describe(current).slots.map(s=>s.id),active:cold.active,bladeId,bowId,sameRunId:bladeRun,veteranId,veteranXP:veteran.adventure.xp,veteranPreserved:true,independentSnapshots:true,sourceWorlds:{blade:'evidence10/pursuit/fresh-blade/run1_03_OBJECTIVES.json',bow:'evidence10/pursuit/fresh-bow/run1_03_OBJECTIVES.json'},canonicalWorldHashes:{blade:crypto.createHash('sha256').update(JSON.stringify(blade)).digest('hex'),bow:crypto.createHash('sha256').update(JSON.stringify(bow)).digest('hex')},commands:{roster:actions.filter(a=>a.type).length,acceptedGame:gameCommands.filter(c=>c.ok).length,refusedGame:gameCommands.filter(c=>!c.ok).length},actions,gameCommands,checks:{cleanCharacterNoProgress:clean.adventure.pursuit.claimed===0,bladeEquipment:blade.adventure.equipment.weapon,bowEquipment:bow.adventure.equipment.weapon,bladePayoutOre:bladePaid-blade.adventure.ore,bowPayoutOre:bowPaid-bow.adventure.ore,reloadActive:cold.active===bowId,outgoingCheckpoint:JSON.stringify(current)===JSON.stringify(coldLoaded.state)}};
