...
function run(){
  ...
  const bladePartial=JSON.parse(fs.readFileSync(path.join(repo,'evidence10/pursuit/fresh-blade/run1_01_STARTED.json')));
  const bowPartial=JSON.parse(fs.readFileSync(path.join(repo,'evidence10/pursuit/fresh-bow/run1_01_STARTED.json')));
  ...
  command('import',{world:blade});const bladeId=store.active;const bladeRun=bladePartial.adventure.pursuit.active.id;snap('02_BLADE_IMPORTED');
  command('delete',{id:'character-1',confirmName:'Visitor'});
  command('import',{world:bow});const bowId=store.active;const bowRun=bowPartial.adventure.pursuit.active.id;snap('03_BOW_IMPORTED');
  ...
  const note={text:'Bow world only',day:current.day};const noteSim=new C.Simulation(current);let nr=noteSim.act('characters-note-'+(++serial),'note',note);assert.equal(nr.ok,true,nr.error);current=noteSim.snapshot();save();
  ...
  assert.equal(current.adventure.equipment.weapon,blade.adventure.equipment.weapon);assert.deepEqual(current.adventure.pursuit.claimed,blade.adventure.pursuit.claimed);
  ...
  assert.deepEqual(current.adventure.pursuit.claimed,bowBeforeSwitch.adventure.pursuit.claimed);assert.equal(current.adventure.pursuit.claimed,bow.adventure.pursuit.claimed);
  ...
  const report={status:'passed',variant:'command-earned-independent-roster',slots:roster.slots.map(s=>s.id),active:cold.active,bladeId,bowId,sameRunId:bladeRun,independentSnapshots:true,sourceWorlds:{blade:'evidence10/pursuit/fresh-blade/07_PRACTICE_PERSISTED.json',bow:'evidence10/pursuit/fresh-bow/07_PRACTICE_PERSISTED.json'},sourceHashes:{blade:crypto.createHash('sha256').update(JSON.stringify(blade)).digest('hex'),bow:crypto.createHash('sha256').update(JSON.stringify(bow)).digest('hex')},commands:actions.length,actions,checks:{cleanCharacterNoProgress:clean.adventure.pursuit.claimed===0,bladeEquipment:blade.adventure.equipment.weapon,bowEquipment:bow.adventure.equipment.weapon,reloadActive:cold.active===bowId,outgoingCheckpoint:JSON.stringify(bowBeforeSwitch)===JSON.stringify(coldLoaded.state)}};
