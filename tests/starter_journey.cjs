...
function veteran(){
 const src=path.join(ROOT,'../veteran-source/HOME_AFTER_BELL_EARNED.json');assert.ok(fs.existsSync(src),'earned Chapter IV fixture');let sim=new C.Simulation(JSON.parse(fs.readFileSync(src))),n=0,actions=[];const out=path.join(ROOT,'veteran');fs.mkdirSync(out,{recursive:true});
 const cmd=(t,p={})=>{let r=sim.adventureCommand('starter-veteran-'+(++n),t,p);assert.ok(r.ok,t+': '+r.error);actions.push({type:t,p});return r;};const tick=t=>{for(let i=0;i<Math.ceil(t/.05);i++)sim.tick(.05);};const walk=(x,z)=>{let r=sim.moveTo(x,z);assert.ok(r.ok,'walk '+x+','+z+': '+r.error);for(let i=0;i<7000&&sim.playerPath.length;i++)tick(.05);assert.ok(Math.hypot(sim.state.player.x-x,sim.state.player.z-z)<.25);actions.push({walk:[x,z]});};const snap=name=>fs.writeFileSync(path.join(out,name+'.json'),JSON.stringify(sim.snapshot(),null,2));
 const veteranWeapon=sim.state.adventure.equipment.weapon;assert.ok(veteranWeapon);const before=A.stats(sim.state.adventure);walk(11,9);cmd('starter-accept');sim=new C.Simulation(sim.snapshot());snap('01_ACCEPTED_RELOADED');walk(15,7);cmd('starter-enter');
 for(const b of Q.BUNDLES){walk(b.x,b.z);cmd('starter-pickup',{id:b.id});}
 const fight=id=>{let e=A.runtime(sim).enemies.find(x=>x.id===id);walk(e.x,e.z);for(let i=0;i<5000&&e.hp>0;i++){let d=Math.hypot(sim.state.player.x-e.x,sim.state.player.z-e.z);if(!sim.playerPath.length&&d>2.1)sim.moveTo(e.x,e.z);if(sim.state.adventure.hp<55&&sim.state.adventure.tonics)cmd('heal');if(d<=2.65&&A.visible(sim,sim.state.player,e)&&sim.state.adventure.elapsed>=A.runtime(sim).cooldowns.attack)cmd('attack',{target:id});if(d<=3.6&&A.visible(sim,sim.state.player,e)&&sim.state.adventure.elapsed>=A.runtime(sim).cooldowns.pulse)cmd('pulse',{target:id});tick(.1);}assert.ok(sim.state.adventure.defeated.includes(id));cmd('target-clear');walk(e.home.x,e.home.z);cmd('loot',{id});};
 fight('river-skitter-west');fight('river-skitter-east');fight('river-old-bristle');walk(0,12);cmd('starter-leave');walk(11,9);cmd('starter-claim',{choice:'temper',weapon:veteranWeapon});const tempered=A.stats(sim.state.adventure);assert.equal(tempered.attack,before.attack+2);assert.equal(tempered.maxHP,before.maxHP);assert.equal(tempered.defense,before.defense);snap('02_TEMPER_CLAIMED');sim=new C.Simulation(sim.snapshot());walk(11,9);assert.equal(sim.state.adventure.starter.reward.weapon,veteranWeapon);assert.equal(A.stats(sim.state.adventure).attack,tempered.attack);cmd('equip',{id:veteranWeapon});snap('03_TEMPER_EQUIPPED_RELOADED');
 const f=sim.snapshot(),report={status:'passed',variant:'veteran',source:'evidence10/veteran-source/HOME_AFTER_BELL_EARNED.json',commands:n,positionEdits:0,inventoryGrants:0,plantedDefeats:0,temperOnly:true,attackBefore:before.attack,attackAfter:tempered.attack,equipment:f.adventure.equipment,chapters:{crossing:f.adventure.crossing,beacon:f.adventure.beacon},actions};fs.writeFileSync(path.join(out,'STARTER_VETERAN_REPORT.json'),JSON.stringify(report,null,2));return report;
}
if(require.main===module&&process.argv.includes('--veteran'))console.log(JSON.stringify(veteran(),null,2));module.exports.veteran=veteran;
