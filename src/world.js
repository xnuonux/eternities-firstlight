...
class WorldArt{
 ...
 person(out,x,z,yaw,color,time,walking,role,player=false,base=1.2,profile=null){let c=hex(color),skin=profile?X.SKINS[profile.skin]:0xc4a58d,wood=0x403d36,add=(kind,a,b,d,sx,sy,sz,col,rx=0,rz=0,em=0)=>{let xx=x+a*Math.cos(yaw)+d*Math.sin(yaw),zz=z-a*Math.sin(yaw)+d*Math.cos(yaw);out[kind].push({p:[xx,base+b,zz],s:[sx,sy,sz],c:col,r:[rx,yaw,rz],em,rough:.75});};let swing=walking?Math.sin(time*8)*.6:Math.sin(time*1.2)*.035,bob=walking?Math.abs(Math.sin(time*8))*.05:0;add('box',0,.78+bob,0,.46,.62,.28,c);add('box',0,1.29+bob,0,.31,.34,.3,skin);add('box',0,1.47+bob,-.02,.35,.13,.32,profile?X.HAIR[profile.hair]:role==='musician'?0x6c5241:role==='researcher'?0xc9c5ad:0x55493c);for(let s of[-1,1]){add('box',s*.31,.86+bob,0,.15,.56,.16,c,swing*s);add('box',s*.14,.29,0,.17,.56,.17,wood,-swing*s);add('box',s*.14,.055,.05,.2,.13,.3,0x393c37);}if(player){add('box',0,.83+bob,-.17,.48,.67,.09,profile?X.CLOAKS[profile.cloak]:0xdbc99c);out.disc.push({p:[x,base+.02,z],s:[1.05,1,1.05],c:0xcebc87,rough:.3,em:.2});}if(role==='musician'){add('octa',.29,.74,.21,.38,.55,.16,0xbd955f);add('box',.38,1.07,.24,.08,.57,.08,0x806747);}if(role==='researcher')add('box',.25,.95,.25,.25,.32,.07,0xb8c8be);}
