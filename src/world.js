...
class WorldArt{
 ...
 person(out,x,z,yaw,color,time,walking,role,player=false,base=1.2,profile=null){let c=hex(color),skin=profile?X.SKINS[profile.skin]:0xc4a58d,wood=0x403d36,add=(kind,a,b,d,sx,sy,sz,col,rx=0,rz=0,em=0)=>{let xx=x+a*Math.cos(yaw)+d*Math.sin(yaw),zz=z-a*Math.sin(yaw)+d*Math.cos(yaw);out[kind].push({p:[xx,base+b,zz],s:[sx,sy,sz],c:col,r:[rx,yaw,rz],em,rough:.75});};let swing=walking?Math.sin(time*8)*.6:Math.sin(time*1.2)*.035,bob=walking?Math.abs(Math.sin(time*8))*.05:0;add('round',0,.78+bob,0,.46,.62,.34,c);add('round',0,1.29+bob,0,.31,.34,.3,skin);add('round',0,1.47+bob,-.02,.35,.15,.32,profile?X.HAIR[profile.hair]:role==='musician'?0x6c5241:role==='researcher'?0xc9c5ad:0x55493c);for(let s of[-1,1]){add('round',s*.31,.86+bob,0,.15,.56,.16,c,0,swing*s);add('round',s*.14,.29,0,.17,.56,.17,wood,0,-swing*s);add('octa',s*.14,.055,.05,.2,.13,.3,0x393c37);}if(player){add('round',0,.83+bob,-.17,.50,.70,.12,profile?X.CLOAKS[profile.cloak]:0xdbc99c);out.disc.push({p:[x,base+.02,z],s:[1.05,1,1.05],c:0xcebc87,rough:.3,em:.2});}if(role==='musician'){add('octa',.29,.74,.21,.38,.55,.16,0xbd955f);add('round',.38,1.07,.24,.08,.57,.08,0x806747);}if(role==='researcher')add('round',.25,.95,.25,.25,.32,.07,0xb8c8be);}
 update(sim,t,target){this.e.cutaway=sim.state.settings.cameraCutaway!==false;this.e.cutawayFocus=[sim.state.player.x,sim.room?2.45:2.18,sim.state.player.z];let out={box:[],octa:[],disc:[],round:[]},p=sim.state.player;this.person(out,p.x,p.z,p.yaw,X.CLOAKS[sim.state.visitor.cloak],t,sim.walking,'visitor',true,this.room?1.57:1.3,sim.state.visitor);for(let i=0;i<C.PROFILES.length;i++){let info=C.PROFILES[i],r=sim.state.residents.find(r=>r.id===info.id),run=sim.runs.get(info.id);if(this.room){if(this.room==='home'&&run.inside){let pos=[[-3.1,1.1],[3.2,1.1],[2,3.2]][i];this.person(out,pos[0],pos[1],i?-.7:.7,info.color,t+i,false,info.role.toLowerCase(),false,1.57);}else if(this.room==='atelier'&&info.id==='ilan'&&run.goal==='atelier')this.person(out,2.5,1.1,-1,info.color,t,false,'musician',false,1.57);else if(this.room==='observatory'&&info.id==='mara'&&run.goal==='observatory')this.person(out,2.5,1.1,-1,info.color,t,false,'researcher',false,1.57);}else if(!run.inside)this.person(out,r.x,r.z,r.yaw,info.color,t+i,run.walking,info.role.toLowerCase());}
 ...
 if(target)for(let i=0;i<16;i++){let a=i/16*TAU;out.box.push({p:[target.x+Math.cos(a)*.4,this.room?1.59:1.31,target.z+Math.sin(a)*.4],s:[.12,.025,.05],r:[0,-a,0],c:0xe5ce93,em:.35});}
