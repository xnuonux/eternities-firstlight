...
class EarthUI{
 constructor(rpg){this.rpg=rpg;this.notes=new G.RealmEarthNotesUI.EarthNotesUI(rpg);this.story=new G.RealmEarthStoryUI.EarthStoryUI(rpg);this.ticket=null;this.reading=null;const root=document.createElement('div');root.id='earth-labels';root.setAttribute('aria-hidden','true');$('#hud').append(root);
 ...
 invitation(){return this.notes.invitation()+'<section class="earth-invitation"><small>EARTH · HEARTHWATER VALE</small><h3>The longer way to useful work</h3><p>Cross the lake footbridge, take the orchard lane to Oren’s riverbank worksite, or follow the ridge toward Bellweather’s bell.</p>'+button('Read the Hearthwater marker','earth-invitation')+'</section>';}
 ...
 tick(){const inside=this.sim.room===N.ROOM;$('#earth-home').hidden=!inside;document.body.classList.toggle('in-earth',inside);const root=$('#earth-labels');root.replaceChildren();
  ...
  if(inside)this.story.track();this.notes.track();
